...
export default async function Image({ params }: { params: Promise<{ id: string }> }) {
  ...
  const eventDate = format(new Date(event.start_at!), 'M月d日(E)', { locale: ja })
  const eventTime = `${format(new Date(event.start_at!), 'HH:mm')} - ${format(new Date(event.end_at!), 'HH:mm')}`
      ...
      >
        ...
        >
          ...
          >
            ...
            <div style={{ color: 'rgba(255, 255, 255, 0.9)', fontSize: '24px' }}>
              宮城バドミントン練習会
            </div>
          </div>
