...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const eventDate = format(new Date(event.start_at!), 'yyyy年M月d日', { locale: ja })
  const levelText = LEVELS[event.level as LevelKey] || event.level
  
  const title = `${event.title} - ${eventDate}`
  const description = `${eventDate}に${event.venue_name}で開催されるバドミントン練習会。レベル: ${levelText}、参加費: ${event.fee?.toLocaleString()}円。${event.description?.slice(0, 100) || ''}`
...
function EventJsonLd({ event }: { event: {
  ...
  const jsonLd = {
    ...
    offers: {
      ...
      price: event.fee,
      priceCurrency: 'JPY',
      availability: event.status === 'published' && (event.remaining_spots ?? 0) > 0
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <nav className="text-sm text-muted mb-6" aria-label="パンくずリスト">
          <ol itemScope itemType="https://schema.org/BreadcrumbList" className="flex">
            ...
            <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
              <span itemProp="name" className="text-gray-900">練習会詳細</span>
