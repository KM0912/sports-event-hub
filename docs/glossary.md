...
**説明**:
イベント一覧から練習会を探し、申請を通じて参加する。同一ユーザーが主催者と参加者の両方の役割を持つことができる（異なるイベントにおいて）。コード上では `applicant` を使用する。
...
**関連用語**: [イベント](#イベント練習会)、[参加者](#参加者)、[主催者](#主催者)
...
**関連用語**: [クイックメッセージ](#クイックメッセージ)、[主催者](#主催者)、[参加者](#参加者)
...
**主要フィールド**:
- `id`: UUID（auth.usersのid）
- `displayName`: 表示名（2〜20文字）
...
- `id`: UUID
- `eventId`: イベントID
- `senderId` / `receiverId`: 送信者・受信者ID
- `content`: メッセージ内容（1〜500文字）
- `isRead`: 既読フラグ
